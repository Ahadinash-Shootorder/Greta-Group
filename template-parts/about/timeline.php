...
:root {
  ...
  --track-h:   560px;
  --mid:       280px;   /* = track-h / 2 */
  --dot-r:     24px;    /* dot radius */
  --dot-d:     48px;    /* dot diameter = 2*dot-r */
  --conn-h:    38px;
}
...
.timeline-sub::before, .timeline-sub::after {
  content: ''; display: block; height: 1px; width: 87px;
}
.timeline-sub::before { background: linear-gradient(90deg, transparent, var(--gold-mid)); }
.timeline-sub::after  { background: linear-gradient(90deg, var(--gold-mid), transparent); }
.timeline-sub p {
  font-style: italic; font-size: 1.05rem;
  color: var(--text-muted); letter-spacing: 0.14em; white-space: nowrap;
}

.chevrons { display: flex; gap: 5px; }
.chevrons i {
...
.chevrons i:nth-child(2) { animation-delay: .18s; opacity: .65; }
.chevrons i:nth-child(3) { animation-delay: .36s; opacity: .35; }
...
.tl-outer::-webkit-scrollbar-track { background: rgba(255,255,255,0.03); border-radius:4px; }
.tl-outer::-webkit-scrollbar-thumb { background: var(--gold-dark); border-radius:4px; }
...
.item.vis { opacity: 1; transform: translateY(0); }

/* stagger */
.item:nth-child(2)  { transition-delay:.05s }
.item:nth-child(3)  { transition-delay:.10s }
.item:nth-child(4)  { transition-delay:.15s }
.item:nth-child(5)  { transition-delay:.20s }
.item:nth-child(6)  { transition-delay:.25s }
.item:nth-child(7)  { transition-delay:.30s }
.item:nth-child(8)  { transition-delay:.35s }
.item:nth-child(9)  { transition-delay:.40s }
.item:nth-child(10) { transition-delay:.45s }
.item:nth-child(11) { transition-delay:.50s }
...
.item:hover .dot::before { opacity: 1; }
...
.item.above:hover .card { transform: translateX(-50%) translateY(-5px); }
.item.below:hover .card { transform: translateX(-50%) translateY(5px); }
...
.item:hover .card::after { transform: translateX(120%); }
...
.card-divider {
  width: 38px; height: 1px; margin: 0 auto 10px;
...
/* FOOTER */
.timeline-footer {
  text-align: center; padding: 8px 0 52px;
  position: relative; z-index: 1;
  opacity: 0; animation: fadeUp .9s ease 1.1s forwards;
}
.timeline-footer-line {
  width: 60px; height: 1px; margin: 0 auto 12px;
  background: linear-gradient(90deg, transparent, var(--gold-mid), transparent);
}
.timeline-footer p {
...
@media (max-width: 600px) {
  ...
  .timeline-heading h1 { font-size: 2.4rem; }
  .tl-outer { padding: 16px 48px 52px; }
  .item { margin: 0 18px; }
  .card-year  { font-size: 1.15rem; }
...
<header class="timeline-heading">
  ...
  <div class="timeline-sub"><p>From Foundation to the Future</p></div>
</header>
